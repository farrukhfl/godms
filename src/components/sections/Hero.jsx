...
export default function Hero() {
  return (
    <section className="relative isolate overflow-hidden bg-gradient-to-br from-white via-mist/60 to-white">
      <div className="absolute -right-48 -top-40 -z-10 h-[38rem] w-[38rem] rounded-full bg-primary/15 blur-3xl" />
      <div className="absolute -bottom-64 left-1/4 -z-10 h-[32rem] w-[32rem] rounded-full bg-accent/10 blur-3xl" />
      ...
      <div className="mx-auto grid min-h-[780px] max-w-7xl items-center gap-16 px-4 py-20 sm:px-6 sm:py-28 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-32">
        <div className="relative z-10">
          <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-primary/15 bg-white/90 px-4 py-2 text-sm font-bold text-primary shadow-soft backdrop-blur">
            <CreditCard aria-hidden="true" size={17} /> A complete POS for one simple monthly price
          </div>
          <h1 className="text-balance text-4xl font-extrabold leading-[0.98] tracking-[-0.055em] text-navy sm:text-6xl lg:text-7xl">
            Switch for <span className="text-primary">$0 upfront.</span> Pay $49.99/month.
          </h1>
          <p className="mt-7 max-w-xl text-lg leading-8 text-slate-600 sm:text-xl sm:leading-9">Get fast approval, no long-term lock-in, and a POS built for retail, restaurants, professional services, healthcare, education, and government organizations.</p>
          ...
          <PricingDisclosure className="mt-5 max-w-lg" />
        </div>

        <div className="relative mx-auto w-full max-w-2xl pb-14 lg:ml-auto lg:scale-105">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-[1.18fr_0.82fr]">
              <div className="rounded-2xl bg-white p-5 sm:p-6">
                ...
                <svg viewBox="0 0 320 100" role="img" aria-label="Sales chart trending upward" className="mt-4 h-24 w-full overflow-visible">
                  ...
                  <path d="M0 83 C35 79 45 58 78 65 S125 76 151 50 S200 62 228 29 S275 42 320 8 L320 100 L0 100 Z" fill="url(#sales-fill)" />
                  <path d="M0 83 C35 79 45 58 78 65 S125 76 151 50 S200 62 228 29 S275 42 320 8" fill="none" stroke="#4880FF" strokeWidth="5" strokeLinecap="round" />
                  <circle cx="320" cy="8" r="6" fill="#f3b61f" stroke="white" strokeWidth="3" />
                </svg>
              ...
              <div className="rounded-2xl bg-slate-50 p-4">
                ...
                {[['Counter sale', '$124.50'], ['Tap payment', '$38.20'], ['Online order', '$86.00'], ['Invoice', '$420.00']].map(([name, amount]) => (
                  <div key={name} className="mt-3 flex items-center justify-between border-t border-slate-200 pt-3 text-[11px]"><span className="text-slate-500">{name}</span><span className="font-bold text-navy">{amount}</span></div>
                ))}
              </div>
            </div>
          </div>
          <div className="absolute bottom-0 right-2 flex w-36 flex-col rounded-[1.35rem] border border-slate-200 bg-white p-3 shadow-2xl sm:-right-8 sm:w-40">
          ...
          <div className="absolute bottom-1 left-2 hidden items-center gap-2 rounded-2xl border border-slate-100 bg-white px-4 py-3 shadow-2xl min-[360px]:flex sm:-left-8">
