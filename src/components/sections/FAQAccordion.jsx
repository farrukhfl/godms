...
export default function FAQAccordion({ questions: customQuestions = questions, title = 'Questions, meet straightforward answers.', description = 'A few helpful details before we start building your payment setup.', variant = 'default' }) {
  ...
  const isAnimated = variant === 'home' || variant === 'industry'
  ...
  return (
    <section id="faqs" className={`${isHome ? 'bg-white py-24 sm:py-32' : 'bg-slate-50 py-20 sm:py-24'}`}>
      <div className={`mx-auto px-4 sm:px-6 lg:px-8 ${isHome ? 'max-w-5xl' : 'max-w-4xl'}`}>
        {isAnimated ? <Reveal><SectionHeading eyebrow="FAQs" title={title} description={description} /></Reveal> : <SectionHeading eyebrow="FAQs" title={title} description={description} />}
        <div className={`${isHome ? 'mt-16 space-y-4' : 'mt-12 space-y-3'}`}>
          {customQuestions.map((item, index) => {
            ...
            const accordionItem = (
              <div className={`overflow-hidden border bg-white transition duration-300 ${isHome ? `rounded-2xl shadow-sm ${isOpen ? 'border-primary/25 shadow-soft' : 'border-slate-200 hover:border-primary/20 hover:shadow-lg'}` : 'rounded-2xl border-slate-200'}`}>
            ...
            return isAnimated ? <Reveal key={item.question} delay={index * 70}>{accordionItem}</Reveal> : <div key={item.question}>{accordionItem}</div>
          })}
        </div>
      </div>
    </section>
