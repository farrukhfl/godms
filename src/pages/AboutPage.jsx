  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-navy pb-0 pt-20 text-white sm:pt-28">
        <div className="absolute -right-32 -top-28 h-96 w-96 rounded-full border-[70px] border-primary/20" />
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl pb-16 sm:pb-20">
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-accent">About Dolphin Merchant Services</p>
            <h1 className="mt-5 text-balance text-6xl font-extrabold tracking-tight sm:text-7xl">Payments Made Better</h1>
            <p className="mt-7 max-w-3xl text-xl leading-9 text-white/85">Reliable payment processing, practical POS technology, and personal support help businesses get paid faster and operate smarter.</p>
          </div>
          <div className="grid gap-px overflow-hidden border-t border-white/20 bg-white/20 sm:grid-cols-2 lg:grid-cols-4">
            {values.map((value) => <div key={value.title} className="bg-primary px-5 py-7">
      ...
      <section id="services" className="bg-slate-50 py-20 sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionHeading eyebrow="Our Services" title="One partner for the ways business moves." description="Start with payments, then connect the technology and services that support what comes next." />
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {orderedServices.map(({ label, path, icon: Icon }) => <Card key={path} className="group flex h-full flex-col">
      ...
      <section className="py-20 sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionHeading eyebrow="Our Values" title="The principles behind every partnership." />
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {principles.map(({ icon: Icon, title, text }, index) => <div key={title} className="border-t-4 border-primary bg-white px-2 py-7 md:px-7">
      ...
      <section className="bg-white py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center justify-between gap-7 rounded-[2rem] bg-primary px-7 py-12 text-center text-white sm:px-12 lg:flex-row lg:text-left">
